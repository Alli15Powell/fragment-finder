...
class Ui_mainWindow(object):
    ...
    def setupUi(self, mainWindow):
        ...
        self.resultsTable.setGeometry(QtCore.QRect(110, 100, 560, 381))
        ...
        self.exitPlotPage.setGeometry(QtCore.QRect(20, 490, 111, 23))
        ...
        self.toDatasetUpload.setGeometry(QtCore.QRect(40, 550, 233, 23))
        ...
        self.toDatabaseUpload.setGeometry(QtCore.QRect(280, 550, 233, 23))
        ...
        self.toResults.setGeometry(QtCore.QRect(520, 550, 233, 23))
        ...
        self.completionTimeLabel.setGeometry(QtCore.QRect(110, 490, 561, 20))
    ...
    def retranslateUi(self, mainWindow):
        ...
        mainWindow.setWindowTitle(_translate("mainWindow", "FragmentFinder(FF)"))
        ...
        self.formatLabel.setText(_translate("mainWindow", "<html><head/><body><p><span style=\" font-size:10pt;\">FASTA and FASTQ Fromats Accepted</span></p></body></html>"))
    ...
    def uploadSeqClicked(self):
        self.seqFile = ""
        # Open file dialog
        fname = QtWidgets.QFileDialog.getOpenFileName(None, "Open File", "", "Text Files (*.txt);;FASTA Files (*.fasta);; FASTQ Files (*.fastq)")
    ...
    def preprocessButtonClicked(self):
        ...
        try:
            self.startPre = time.perf_counter()
            self.fasta_read_count = Procedure.separate_seqs(self.dbFile, self.seqFile, self.tempSeparatedFile, 18)
            self.endPre = time.perf_counter()
        except:
            traceback.print_exc()
    ...
    def processButtonClicked(self):
        ...
        self.results = {}
        try:
            self.startPrc = time.perf_counter()
            self.results = Procedure.seq_aligner(self.dbFile, self.tempSeparatedFile, 18)
            self.endProc = time.perf_counter()
        except:
            traceback.print_exc()
        totalTime = (self.endPre-self.startPre) + (self.endProc-self.startPrc)
        #print(totalTime)
        ...
        try:
            ...
            get_peaks(self.results, self.orig_strings, self.fasta_read_count, self.exportedResultsSeq, self.exportedResultsDB)
        except:
            traceback.print_exc("could not align")
    ...
    def val_check(self, arr, index, threshold, peak):
        result = []
        right_indeces = []
        left_indeces = []
    ...
    def regen_vector(self, val):
        t = val
        standard_deviation = np.std(val)
        peaks, _ = find_peaks(val, prominence=standard_deviation*0.85, distance=20)
        for peak in peaks:
            if ((val[peak] / self.fasta_read_count) * 1000000) >= 5.0:
                print(val[peak] - (standard_deviation*0.1))
                peak_slice = self.val_check(val, peak, (val[peak]-(standard_deviation*0.1)), val[peak])
                peak_start = peak_slice[0]
                peak_end = peak_slice[-1]
                print(f'{peak_start}:{peak_end}')
                t[peak_start:peak_end+18] = val[peak]
            else:
                continue
        return t
    ...
    def plotButtonClicked(self):
        ...
        selection = self.geneSearchBox.text()
        self.sc.axes.cla()

        if not selection == '':
            try:
                vector = self.results[selection]
                standard_deviation = np.std(vector)
                peaks, _ = find_peaks(vector, prominence=standard_deviation*0.9, distance=20)
                self.uidLabel.setText(selection)
                for peak in peaks:
                    if ((vector[peak] / self.fasta_read_count) * 1000000) <= 5.0:
                        peaks.remove(peak)

                x = vector.copy()
                v = self.regen_vector(x)
                self.sc.axes.plot(v, label='Expression')
                self.sc.axes.plot(peaks, vector[peaks], 'ko', label='Peak Start')

                z = vector.copy()

                for peak in peaks:
                    if ((vector[peak] / self.fasta_read_count) * 1000000) >= 5.0:
                        peak_slice = self.val_check(z, peak, (z[peak]-(standard_deviation*0.1)), z[peak])
                        peak_start = peak_slice[0]
                        peak_end = peak_slice[-1]
                        z[peak_start:peak_end+18] = z[peak]
                        self.sc.axes.vlines(x=[peak_start, peak_end+18], ymin=0, ymax=z[peak], color='g', linestyle='dashed')
                        self.sc.axes.fill_betweenx([0, z[peak]], peak_start, peak_end+18, color='g', alpha=0.2)
                    else:
                        continue
                self.sc.axes.legend()
            ...
            try:
                for idx in self.resultsTable.selectedItems():
                    uid = str(idx.text())
                    vector = self.results[idx.text()]
                self.uidLabel.setText(uid)
                standard_deviation = np.std(vector)
                peaks, _ = find_peaks(vector, prominence=standard_deviation*0.85, distance=20)
                self.uidLabel.setText(selection)
                x = vector.copy()
                v = self.regen_vector(x)
                self.sc.axes.plot(v, label='Expression')
                ...
                for peak in peaks:
                    if ((vector[peak] / self.fasta_read_count) * 1000000) >= 5.0:
                        self.sc.axes.plot(peak, z[peak], 'ko', label='Peaks')
                        #print(vector[peak] - (standard_deviation*0.1))
                        peak_slice = self.val_check(z, peak, (z[peak]-(standard_deviation*0.1)), z[peak])
                        peak_start = peak_slice[0]
                        peak_end = peak_slice[-1]
                        #print(f'{peak_start}:{peak_end}')
